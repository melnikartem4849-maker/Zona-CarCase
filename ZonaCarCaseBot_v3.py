...
# Базовый список автомобилей удалён намеренно.
# Все автомобили добавляются создателем через админ-панель.
...
def load_custom_cars():
    ...
    # В игре только машины, добавленные через админ-панель.
    CARS = custom
...
def choose_car():
    if not CARS:
        return None
    available = [r for r in RARITY_ORDER if CARS_BY_RARITY[r]]
    weights = [RARITIES[r]["chance"] for r in available]
    rarity = random.choices(available, weights=weights, k=1)[0]
    return random.choice(CARS_BY_RARITY[rarity])


def choose_container_car(container_id):
    c = CONTAINERS[container_id]
    available = [r for r in c["rarities"] if CARS_BY_RARITY[r]]
    if not available:
        return None
...
def start_new_auction():
    if not CARS_BY_RARITY["Exclusive"]:
        return None
...
async def open_case(callback: CallbackQuery):
    ...
    car = choose_car()
    if not car:
        await callback.answer("🚘 Пока нет автомобилей. Создатель добавит их через админ-панель.", show_alert=True)
        return
    with db() as conn:
...
async def main():
    ...
    if CARS_BY_RARITY["Exclusive"] and not get_auction():
        start_new_auction()
